...
/**
 * In this program we implemented the prototype of R-MESHJOIN algorithm. The distribution
 *  of memory to the different components of R-MESHJOIN is according to the formulas we mentioned in the paper. However you
 *  have to set these values for every new memory budget for R-MESHJOIN.
 * The further specification of components is as under:
 * 1. DISK_TUPLES
 * : These are the total number of tuples in relation R, stored on disk and accessed sequentially but
 * in cyclic manner. In our implementation we have used 0.5 to 8 million tuples stored in R, however we also tested the performance of R-MESHJOIN with different values of disk-based relation size.
 * 2. DISK-BUFFER
 * We have tuned the disk-buffer for every new budget using program "FindOptimalbForRMeshJoin.java" and used the optimal value for disk-buffer for each scenario.
 * 3. STREAM-BUFFER
 ...
 * 4. HASH_TUPLES
 * The total number of hash tuples can be calculated by knowing its size as described in paper.
 ...
 * The number of cells in the queue is equal to the product of logical divisions 'l' in the disk-buffer and the number of iterations 
 * required to bring whole R into memory. 
 * 
 ...
 *
 */
public class RMeshJoinWithRinDB {
	//****************RELATION R******************
	public static final int R_SIZE=2000000;
	public static final int DISK_BUFFER=2748;
	public static final int QUEUE_SIZE=7278;

	//************STREAM-BUFFER********************
	public static final int STREAM_BUFFER=44;
	...
	public void lookupMasterData()throws java.io.IOException, InterruptedException{
		...
		int matched=0,diskInputs=0, streamInputs=0;
		...
		//System.out.println("Matched tuples:  "+matched);
		//System.out.println("Probed Tuples: "+numberOfProbTuples);
		...
		System.out.println("Number of stream inputs: "+streamInputs);
	...
	public void removeExpireTuples(){
		...
		CE[CE_index++]=CE_per_Iteration/STREAM_BUFFER;
	}
	/**
	 * This method read the fixed number of tuples from disk to disk_buffer. 
	...
	/**
	 * The main method initialized different arrays and call the above mentioned 
	...
	public static void main (String args[])throws java.io.IOException, InterruptedException{
		...
		RMeshJoinWithRinDB rmj=new RMeshJoinWithRinDB();
		System.out.println("Processing Continue...");
		rmj.lookupMasterData();

		System.out.println("R-MESHJOIN With fixed memory");
		System.out.println("\nMEMORY STATUS");
		...
		BufferedWriter bw=new BufferedWriter(new FileWriter("F://Performance comp using realistic benchmark//R-MESHJOIN//M varies//Cloop_Cost_Lamda=2000_R=2m_M=50MB.txt"));
		System.out.println("\nTIME STATUS");
		bw.write("R-MESHJOIN PROCESSING COST");
		bw.newLine();
		bw.write("CH(NSec)\t   CS(NSec)\t     CA(NSec)\t    CE(NSec)\t       CIO(NSec)");
		bw.newLine();

		for(int i=0; i<RMeshJoinWithRinDB.CIO_index; i++){
			bw.write(RMeshJoinWithRinDB.CH[i]+"\t\t");
			bw.write(RMeshJoinWithRinDB.CS[i]+"\t\t");
			bw.write(RMeshJoinWithRinDB.CA[i]+"\t\t");
			bw.write(RMeshJoinWithRinDB.CE[i]+"\t\t");
			bw.write(RMeshJoinWithRinDB.CIO[i]/10+"");
			bw.newLine();
		}
		bw.close();
